...
program
    ...
    const runner = new MigrationRunner(options.rpc, options.network, secret, contractId);
    try {
...
program
    ...
    const secret = options.secret || process.env.SOROBAN_ADMIN_SECRET;
    const runner = new MigrationRunner(options.rpc || process.env.SOROBAN_RPC_URL || 'https://soroban-testnet.stellar.org', options.network || process.env.SOROBAN_NETWORK_PASSPHRASE || 'Test SDF Network ; September 2015', secret, contractId);
    try {
